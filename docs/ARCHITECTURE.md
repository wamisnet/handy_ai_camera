...
```
 ...
 Frame(bgr, flash)  ×  フラッシュシーケンス [OFF, ON] × framesPerStep
        │
        ▼
 (ガイド枠でクロップ) → グレー箱のみ BoxRectifier: 一番上の箱の内側矩形を検出 → 射影変換で正面視に補正
        │
        ▼
 Segmenter ─── 黒背景: Otsu ∧ フラッシュ差分 / グレー箱: 平面フィット背景からの Lab 距離 ─── → 前景マスク・物体一覧
...
| 背景の映り込み | フラッシュ差分(近い物体だけ明るくなる) ∧ Otsu、面積フィルタ。グレー箱は箱の内側矩形に解析を限定し、周辺リングから背景を推定 |
| 箱の積み重ね・隣接、内壁の影 | BoxRectifier が「エッジに囲まれた、画像の縁に触れない最大の低勾配領域」を一番上の箱の内側として検出し、その内側だけを解析。背景の明度は平面フィットで影の勾配を吸収 |
| 期待より多い物体(縁・隣の部品) | rejectExtraObjects(既定 ON): 期待数より多く検出したフレームは不採用。面積上位を黙って採用しない |
...
- **箱の内側矩形検出**(`imaging/BoxRectifier.kt`): 勾配しきい値(60)・最小面積比(15%)・縁マージン(6px)を定数で持つ。実物の箱で縁のコントラストが弱い場合はここを調整する。検証は `pctools/boxsim.py`
